...
const { buyTollRoadCard, topUpBalance, useTollRoad } = require('./utils.js');

async function getTollAccess(money, amount) {
  try {
    const newCard = await buyTollRoadCard(money);
    const cardWithBalance = await topUpBalance(newCard, amount);
    const result = await useTollRoad(cardWithBalance);
...
getTollAccess(100, 50)  // No .then() syntax since each step only prints message to console and doesn't return any specific data
